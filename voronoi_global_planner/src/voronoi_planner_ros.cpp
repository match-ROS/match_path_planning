#include <voronoi_global_planner/voronoi_planner_ros.h>
#include <pluginlib/class_list_macros.h>

// register this planner as a BaseGlobalPlanner plugin
PLUGINLIB_EXPORT_CLASS(global_planner::VoronoiPlannerROS, nav_core::BaseGlobalPlanner)
...
{
    ROS_INFO("VoronoiPlannerROS empty Constructor got called!");
}

VoronoiPlannerROS::VoronoiPlannerROS(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
{
    ROS_INFO("VoronoiPlannerROS Constructor got called!");
    initialize(name, costmap_ros);
}

VoronoiPlannerROS::~VoronoiPlannerROS()
{
    ROS_INFO("VoronoiPlannerROS Deconstructor called");
    if (this->dsrv_)
    {
        delete this->dsrv_;
    }
...
void VoronoiPlannerROS::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros)
{
    if (!initialized_)
    {
        ROS_INFO("VoronoiPlannerROS: Init got called!");
        ...
        this->make_plan_service_ = private_nh.advertiseService("make_plan", &VoronoiPlannerROS::makePlanService, this);
        this->make_plan_with_stats_service_ = private_nh.advertiseService("make_plan_with_stats", &VoronoiPlannerROS::makePlanWithStatsService, this);

        this->dsrv_ = new dynamic_reconfigure::Server<voronoi_global_planner::VoronoiPlannerConfig>(ros::NodeHandle("~/" + name));
        dynamic_reconfigure::Server<voronoi_global_planner::VoronoiPlannerConfig>::CallbackType cb = boost::bind(&VoronoiPlannerROS::reconfigureCB, this, _1, _2);
    ...
    {
        ROS_ERROR("VoronoiPlannerROS already initialized, aborting init!");
    }
}

void VoronoiPlannerROS::reconfigureCB(voronoi_global_planner::VoronoiPlannerConfig &config, uint32_t level)
{
...
bool VoronoiPlannerROS::cancel()
{
    ROS_ERROR("VoronoiPlannerROS CANCEL");
    // Returns false if not implemented. Returns true if implemented and cancel has been successful.
    return false;
}


bool VoronoiPlannerROS::makePlanWithStatsService(voronoi_global_planner::MakePlanWithStats::Request& req, voronoi_global_planner::MakePlanWithStats::Response& res)
{
...
bool VoronoiPlannerROS::makePlanService(navfn::MakeNavPlan::Request& req, navfn::MakeNavPlan::Response& res)
{
    std::vector<geometry_msgs::PoseStamped> path;
                                    ...
                                    std::vector<geometry_msgs::PoseStamped>& plan)
{
    ...
    ROS_INFO("VoronoiPlannerROS makePlan got called!");
    if (!initialized_)
    {
        ROS_ERROR(
    ...
    // ROS_INFO_STREAM("VoronoiPlannerROS: Time for start and goal coordinate transforms (s): " << coord_trans_duration);
    ...
    // ROS_INFO_STREAM("VoronoiPlannerROS: Time taken for putting costmap in image (s): " << costmap_as_img_duration);
    ...
    ROS_INFO_STREAM("VoronoiPlannerROS: Time for generating voronoi img (s): " << voronoi_generation_duration);
    ...
    ROS_INFO_STREAM("VoronoiPlannerROS: Time for generating voronoi map (s): " << voronoi_map_duration);

    // get path on voronoi image
    std::vector<cv::Point2i> path;
    bool path_found = global_planner::findCompletePath(this->obstacle_img_, this->voronoi_img_, path, this->start_map_, this->goal_map_);
    ...
    ROS_INFO_STREAM("VoronoiPlannerROS: Time taken for finding path (s): " << path_finding_duration);
    ...
    global_planner::createPlanFromPath(path, unreduced_plan, this->costmap_, this->costmap_global_frame_);
    ...
    // bool sparse_success = global_planner::sparsify_path(path, sparse_path, this->angle_threshold_, min_distance_control_points_px);

    // // create sparse plan for debug output
    // std::vector<geometry_msgs::PoseStamped> sparse_plan;
    // global_planner::createPlanFromPath(sparse_path, sparse_plan, this->costmap_, this->costmap_global_frame_);
    ...
    // global_planner::pathMapToWorld(sparse_path, sparse_path_world, this->costmap_);
    ...
    // // ROS_INFO_STREAM("VoronoiPlannerROS: Time taken to create plan from path (s): " << plan_creating_duration);
    ...
    // global_planner::createPlanFromPath(spline_samples_initial, spline_plan_initial, this->costmap_global_frame_);
    ...
    // global_planner::createPlanFromPath(optimized_sparse_path, optimized_sparse_plan, this->costmap_global_frame_);
    ...
    // global_planner::createPlanFromPath(continuous_path, splined_plan, this->costmap_global_frame_);
    ...
    // ROS_INFO_STREAM("VoronoiPlannerROS: Time taken for building splined plan (s): " << splining_duration);

    // if (!global_planner::isPlanFree(costmap_, free_cell_threshold_, splined_plan))
    // {
    ...
    // ROS_INFO_STREAM(
    //     "VoronoiPlannerROS: Time taken for checking if final plan is free (s): "
    ...
    // ROS_INFO_STREAM("VoronoiPlannerROS: Total Time taken (s): " << total_time_taken);
...
void VoronoiPlannerROS::publishPlan(const std::vector<geometry_msgs::PoseStamped>& path, const ros::Publisher& pub)
{
    if (!initialized_)
    {
        ROS_ERROR(
